...
class topology_coordinator : public endpoint_lifecycle_subscriber {
    ...
    future<group0_guard> cleanup_group0_config_if_needed(group0_guard guard) {
        ...
        if (!rconf.is_joint()) {
            ...
            if (!to_remove.empty()) {
                ...
                try {
                    LOGMACRO(rtlogger, log_level::debug, "topology coordinator fiber removing {}"
    ...
    future<group0_guard> start_operation() {
        LOGMACRO(rtlogger, log_level::debug, "obtaining group 0 guard...");
        auto guard = co_await _group0.client().start_operation(_as);
        LOGMACRO(rtlogger, log_level::debug, "guard taken, prev_state_id: {}, new_state_id: {}, coordinator term: {}, current Raft term: {}",
    ...
    future<> update_topology_state(
        ...
        try {
            LOGMACRO(rtlogger, log_level::info, "updating topology state: {}", reason);
            LOGMACRO(rtlogger, log_level::trace, "update_topology_state mutations: {}", updates);
            topology_change change{std::move(updates)};
            group0_command g0_cmd = _group0.client().prepare_command(std::move(change), guard, reason);
            co_await _group0.client().add_entry(std::move(g0_cmd), std::move(guard), _as);
        } catch (group0_concurrent_modification&) {
            LOGMACRO(rtlogger, log_level::info, "race while changing state: {}. Retrying", reason);
            throw;
        }
    };
    ...
    future<> exec_direct_command_helper(raft::server_id id, uint64_t cmd_index, const raft_topology_cmd& cmd) {
        LOGMACRO(rtlogger, log_level::debug, "send {} command with term {} and index {} to {}",
    ...
    future<group0_guard> exec_global_command(
        ...
        LOGMACRO(rtlogger, log_level::info, "executing global topology command {}, excluded nodes: {}", cmd.cmd, exclude_nodes);
    ...
    future<group0_guard> remove_from_group0(group0_guard guard, const raft::server_id& id) {
        LOGMACRO(rtlogger, log_level::info, "removing node {} from group 0 configuration...", id);
        release_guard(std::move(guard));
        co_await _voter_handler.on_node_removed(id, _as);
        co_await _group0.remove_from_raft_config(id);
        LOGMACRO(rtlogger, log_level::info, "node {} removed from group 0 configuration", id);
    ...
    future<> cdc_generation_publisher_fiber() {
        LOGMACRO(rtlogger, log_level::debug, "start CDC generation publisher fiber");

        while (!_as.abort_requested()) {
            co_await utils::get_local_injector().inject("cdc_generation_publisher_fiber", [] (auto& handler) -> future<> {
                LOGMACRO(rtlogger, log_level::info, "CDC generation publisher fiber sleeps after injection");
                co_await handler.wait_for_message(std::chrono::steady_clock::now() + std::chrono::minutes{5});
                LOGMACRO(rtlogger, log_level::info, "CDC generation publisher fiber finishes sleeping after injection");
            }, false);
            ...
            try {
                ...
                if (_topo_sm._topology.unpublished_cdc_generations.empty()) {
                    ...
                    LOGMACRO(rtlogger, log_level::debug, "CDC generation publisher fiber has nothing to do. Sleeping.");
                    co_await _topo_sm.event.when([&] () {
                        return !_topo_sm._topology.unpublished_cdc_generations.empty() || _as.abort_requested();
                    });
                    LOGMACRO(rtlogger, log_level::debug, "CDC generation publisher fiber wakes up");
                }
            } catch (raft::request_aborted&) {
                LOGMACRO(rtlogger, log_level::debug, "CDC generation publisher fiber aborted");
            } catch (seastar::abort_requested_exception&) {
                LOGMACRO(rtlogger, log_level::debug, "CDC generation publisher fiber aborted");
            } catch (group0_concurrent_modification&) {
            } catch (term_changed_error&) {
                LOGMACRO(rtlogger, log_level::debug, "CDC generation publisher fiber notices term change {} -> {}", _term, _raft.get_current_term());
            } catch (...) {
            ...
            if (sleep) {
                try {
                    ...
                    LOGMACRO(rtlogger, log_level::debug, "CDC generation publisher: sleep failed: {}", std::current_exception());
                }
            }
    ...
    future<> gossiper_orphan_remover_fiber() {
        LOGMACRO(rtlogger, log_level::debug, "start gossiper orphan remover fiber");
        ...
        while (!_as.abort_requested()) {
            try {
                ...
                LOGMACRO(rtlogger, log_level::debug, "gossiper orphan remover fiber aborted");
            } catch (seastar::abort_requested_exception&) {
                LOGMACRO(rtlogger, log_level::debug, "gossiper orphan remover fiber aborted");
            } catch (group0_concurrent_modification&) {
            } catch (term_changed_error&) {
                LOGMACRO(rtlogger, log_level::debug, "gossiper orphan remover fiber notices term change {} -> {}", _term, _raft.get_current_term());
            } catch (...) {
            ...
            try {
                ...
                LOGMACRO(rtlogger, log_level::debug, "gossiper orphan remover: sleep failed: {}", std::current_exception());
            }
            co_await coroutine::maybe_yield();
        }
    }
    ...
    future<> group0_voter_refresher_fiber() {
        LOGMACRO(rtlogger, log_level::debug, "start group0 members refresh fiber");
        ...
        while (!_as.abort_requested()) {
            try {
                ...
                if (!_feature_service.group0_limited_voters) {
                    LOGMACRO(rtlogger, log_level::debug, "group0 voters refresh fiber iteration skipped because the feature is disabled");
                    continue;
                }

                co_await _voter_handler.refresh(_as);
            } catch (raft::request_aborted&) {
                LOGMACRO(rtlogger, log_level::debug, "group0 voters refresh fiber aborted");
                break; // exit the loop immediately (not waiting for the abort source to be set)
            } catch (seastar::abort_requested_exception&) {
                LOGMACRO(rtlogger, log_level::debug, "group0 voters refresh fiber aborted");
                break; // exit the loop immediately (not waiting for the abort source to be set)
            } catch (group0_concurrent_modification&) {
                LOGMACRO(rtlogger, log_level::debug, "group0 voters refresh fiber notices concurrent modification, retrying...");
                skip_wait = true;
            } catch (term_changed_error&) {
                LOGMACRO(rtlogger, log_level::debug, "group0 voters refresh fiber notices term change {} -> {}", _term, _raft.get_current_term());
    ...
    future<> handle_global_request(group0_guard guard) {
        ...
        case global_topology_request::new_cdc_generation: {
            LOGMACRO(rtlogger, log_level::info, "new CDC generation requested");
        ...
        case global_topology_request::keyspace_rf_change: {
            LOGMACRO(rtlogger, log_level::info, "keyspace_rf_change requested");
        ...
        case global_topology_request::truncate_table: {
            LOGMACRO(rtlogger, log_level::info, "TRUNCATE TABLE requested");
            std::vector<canonical_mutation> updates;
    ...
    future<> enable_features(group0_guard guard, std::set<sstring> features_to_enable) {
        ...
        LOGMACRO(rtlogger, log_level::info, "enabled features: {}", features_to_enable);
    }
    ...
    bool advance_in_background(locator::global_tablet_id gid, background_action_holder& holder, const char* name,
        ...
        if (!holder || holder->failed()) {
            ...
            holder = futurize_invoke(action).then_wrapped([this, gid, name] (future<> f) {
                ...
                LOGMACRO(rtlogger, log_level::debug, "{} for tablet {} resolved.", name, gid);
                _tablets_ready = true;
        ...
        if (!holder->available()) {
            LOGMACRO(rtlogger, log_level::debug, "Tablet {} still doing {}", gid, name);
            return false;
        }
    ...
    void generate_resize_update(std::vector<canonical_mutation>& out, const group0_guard& guard, table_id table_id, locator::resize_decision resize_decision) {
            ...
            LOGMACRO(rtlogger, log_level::debug, "Generating resize decision for table {} of type {} and sequence number {}",
    ...
    future<> handle_tablet_migration(group0_guard guard, bool drain) {
        ...
        LOGMACRO(rtlogger, log_level::debug, "handle_tablet_migration()");
        std::vector<canonical_mutation> updates;
        ...
        co_await for_each_tablet_transition([&] (const locator::tablet_map& tmap,
            ...
            auto transition_to = [&] (locator::tablet_transition_stage stage) {
                LOGMACRO(rtlogger, log_level::debug, "Will set tablet {} stage to {}", gid, stage);
            ...
            auto check_excluded_replicas = [&] {
                    ...
                    for (auto r : tsi.read_from) {
                        if (is_excluded(raft::server_id(r.host.uuid()))) {
                            LOGMACRO(rtlogger, log_level::debug, "Aborting streaming of {} because read-from {} is marked as ignored", gid, r);
                            return true;
                        }
                    }
                    for (auto r : tsi.written_to) {
                        if (is_excluded(raft::server_id(r.host.uuid()))) {
                            LOGMACRO(rtlogger, log_level::debug, "Aborting streaming of {} because written-to {} is marked as ignored", gid, r);
                            return true;
                        }
                    }
                    return false;
            ...
            switch (trinfo.stage) {
                case locator::tablet_transition_stage::allow_write_both_read_old:
                    ...
                    if (do_barrier()) {
                        LOGMACRO(rtlogger, log_level::debug, "Will set tablet {} stage to {}", gid, locator::tablet_transition_stage::write_both_read_old);
                        updates.emplace_back(get_mutation_builder()
                ...
                case locator::tablet_transition_stage::rebuild_repair: {
                    if (action_failed(tablet_state.rebuild_repair)) {
                        ...
                        if (fail || check_excluded_replicas()) {
                            if (do_barrier()) {
                                LOGMACRO(rtlogger, log_level::debug, "Will set tablet {} stage to {}", gid, locator::tablet_transition_stage::cleanup_target);
                                updates.emplace_back(get_mutation_builder()
                    ...
                    if (advance_in_background(gid, tablet_state.rebuild_repair, "rebuild_repair", [&] {
                        ...
                        if (!trinfo.pending_replica) {
                            LOGMACRO(rtlogger, log_level::info, "Skipped tablet rebuild repair of {} as no pending replica found", gid);
                            return make_ready_future<>();
                        }
                        auto tsi = get_migration_streaming_info(get_token_metadata().get_topology(), tmap.get_tablet_info(gid.tablet), trinfo);
                        if (tsi.read_from.empty()) {
                            LOGMACRO(rtlogger, log_level::info, "Skipped tablet rebuild repair of {} as no tablet replica was found", gid);
                            return make_ready_future<>();
                        }
                        ...
                        LOGMACRO(rtlogger, log_level::info, "Initiating repair phase of tablet rebuild host={} tablet={}", dst, gid);
                        return ser::storage_service_rpc_verbs::send_tablet_repair(&_messaging,
                                dst, _as, raft::server_id(dst.uuid()), gid).discard_result();
                    })) {
                        LOGMACRO(rtlogger, log_level::debug, "Will set tablet {} stage to {}", gid, locator::tablet_transition_stage::streaming);
                        updates.emplace_back(get_mutation_builder()
                ...
                case locator::tablet_transition_stage::streaming: {
                    ...
                    if (!wait && advance_in_background(gid, tablet_state.streaming, "streaming", [&] {
                        ...
                        if (!trinfo.pending_replica) {
                            LOGMACRO(rtlogger, log_level::info, "Skipped tablet streaming ({}) of {} as no pending replica found", trinfo.transition, gid);
                            return make_ready_future<>();
                        }
                        LOGMACRO(rtlogger, log_level::info, "Initiating tablet streaming ({}) of {} to {}", trinfo.transition, gid, *trinfo.pending_replica);
                        ...
                        LOGMACRO(rtlogger, log_level::debug, "Will set tablet {} stage to {}", gid, locator::tablet_transition_stage::write_both_read_new);
                        updates.emplace_back(get_mutation_builder()
                ...
                case locator::tablet_transition_stage::write_both_read_new: {
                    utils::get_local_injector().inject("crash-in-tablet-write-both-read-new", [] {
                        LOGMACRO(rtlogger, log_level::info, "crash-in-tablet-write-both-read-new hit, killing the node");
                        _exit(1);
                    });
                    ...
                    if (action_failed(tablet_state.barriers[trinfo.stage])) {
                        ...
                        if (excluded_new > excluded_old && trinfo.transition != locator::tablet_transition_kind::intranode_migration) {
                            LOGMACRO(rtlogger, log_level::debug, "During {} stage of {} {} new nodes and {} old nodes were excluded", trinfo.stage, gid, excluded_new, excluded_old);
                ...
                case locator::tablet_transition_stage::cleanup:
                    if (advance_in_background(gid, tablet_state.cleanup, "cleanup", [&] {
                        ...
                        if (!maybe_dst) {
                            LOGMACRO(rtlogger, log_level::info, "Tablet cleanup of {} skipped because no replicas leaving", gid);
                            return make_ready_future<>();
                        }
                        locator::tablet_replica& dst = *maybe_dst;
                        if (is_excluded(raft::server_id(dst.host.uuid()))) {
                            LOGMACRO(rtlogger, log_level::info, "Tablet cleanup of {} on {} skipped because node is excluded", gid, dst);
                            return make_ready_future<>();
                        }
                        LOGMACRO(rtlogger, log_level::info, "Initiating tablet cleanup of {} on {}", gid, dst);
                ...
                case locator::tablet_transition_stage::cleanup_target:
                    if (advance_in_background(gid, tablet_state.cleanup, "cleanup_target", [&] {
                        if (!trinfo.pending_replica) {
                            LOGMACRO(rtlogger, log_level::info, "Tablet cleanup of {} skipped because no replicas pending", gid);
                            return make_ready_future<>();
                        }
                        locator::tablet_replica dst = *trinfo.pending_replica;
                        if (is_excluded(raft::server_id(dst.host.uuid()))) {
                            LOGMACRO(rtlogger, log_level::info, "Tablet cleanup of {} on {} skipped because node is excluded and doesn't need to revert migration", gid, dst);
                            return make_ready_future<>();
                        }
                        LOGMACRO(rtlogger, log_level::info, "Initiating tablet cleanup of {} on {} to revert migration", gid, dst);
                        return ser::storage_service_rpc_verbs::send_tablet_cleanup(&_messaging,
                                                                                   dst.host, _as, raft::server_id(dst.host.uuid()), gid);
                    })) {
                ...
                case locator::tablet_transition_stage::repair: {
                    ...
                    if (advance_in_background(gid, tablet_state.repair, "repair", [&] () -> future<> {
                        ...
                        if (!valid) {
                            LOGMACRO(rtlogger, log_level::info, "Skipping tablet repair for tablet={} which is cancelled by user", gid);
                            co_return;
                        }
                        ...
                        LOGMACRO(rtlogger, log_level::info, "Initiating tablet repair host={} tablet={}", dst, gid);
                        ...
                        LOGMACRO(rtlogger, log_level::info, "Finished tablet repair host={} tablet={} duration={} repair_time={}",
                        ...
                        LOGMACRO(rtlogger, log_level::debug, "Will set tablet {} stage to {}", gid, locator::tablet_transition_stage::end_repair);
                        ...
                        if (valid && is_filter_off) {
                            ...
                            LOGMACRO(rtlogger, log_level::debug, "Set tablet repair time sched_time={} return_time={} set_time={}",
        ...
        if (has_transitions) {
            ...
            if (!_tablets_ready) {
                LOGMACRO(rtlogger, log_level::debug, "Going to sleep with active tablet transitions");
                release_guard(std::move(guard));
                co_await await_event();
            }
            co_return;
    ...
    future<> handle_truncate_table(group0_guard guard) {
        ...
        if (_topo_sm._topology.session) {
            ...
            if (table) {
                ...
                LOGMACRO(rtlogger, log_level::info, "Performing TRUNCATE TABLE for {}.{}", ks_name, cf_name);
        ...
        utils::get_local_injector().inject("truncate_crash_after_session_clear", [] {
            LOGMACRO(rtlogger, log_level::info, "truncate_crash_after_session_clear hit, killing the node");
            _exit(1);
        });
    ...
    future<bool> handle_topology_transition(group0_guard guard) {
        ...
        LOGMACRO(rtlogger, log_level::info, "entered `{}` transition state", *tstate);
        switch (*tstate) {
            case topology::transition_state::join_group0: {
                ...
                if (!accepted) {
                    ...
                    LOGMACRO(rtlogger, log_level::info, "node {} moved to left state", node.id);
            ...
            case topology::transition_state::left_token_ring: {
                ...
                if (node.id == _raft.id()) {
                    ...
                    LOGMACRO(rtlogger, log_level::info, "coordinator is decommissioning; giving up leadership");
            ...
            case topology::transition_state::rollback_to_normal: {
                ...
                LOGMACRO(rtlogger, log_level::info, "{}", str);
    ...
    future<> handle_node_transition(node_to_work_on&& node) {
        LOGMACRO(rtlogger, log_level::info, "coordinator fiber found a node to work on id={} state={}", node.id, node.rs->state);

        switch (node.rs->state) {
            case node_state::none: {
                if (_topo_sm._topology.normal_nodes.empty()) {
                    LOGMACRO(rtlogger, log_level::info, "skipping join node handshake for the first node in the cluster");
                } else {
                    ...
                    if (std::holds_alternative<join_node_response_params::rejected>(validation_result)) {
                        ...
                        LOGMACRO(rtlogger, log_level::info, "rejected node moved to left state {}", node.id);
    ...
    std::vector<canonical_mutation> mark_nodes_as_cleanup_needed(node_to_work_on& node, bool rollback) {
        ...
        for (auto& n : dirty_nodes) {
            ...
            if (node.id != id) {
                ...
                muts.emplace_back(builder.build());
                LOGMACRO(rtlogger, log_level::debug, "mark node {} as needed for cleanup", id);
            }
        }
        return muts;
    }
    ...
    future<> start_cleanup_on_dirty_nodes(group0_guard guard, bool global_request) {
        ...
        for (auto& [id, rs] : topo.normal_nodes) {
            if (rs.cleanup == cleanup_status::needed) {
                ...
                muts.emplace_back(builder.build());
                LOGMACRO(rtlogger, log_level::debug, "mark node {} as cleanup running", id);
            }
        }
        if (!muts.empty()) {
...
future<std::optional<group0_guard>> topology_coordinator::maybe_migrate_system_tables(group0_guard guard) {
    ...
    if (view_builder_version == db::system_keyspace::view_builder_version_t::v1 && _feature_service.view_build_status_on_group0) {
        LOGMACRO(rtlogger, log_level::info, "Migrating view_builder to v1_5");
        auto tmptr = get_token_metadata_ptr();
    ...
    if (view_builder_version == db::system_keyspace::view_builder_version_t::v1_5) {
        if (!get_dead_nodes().empty()) {
            LOGMACRO(rtlogger, log_level::debug, "Not all nodes are alive. Skipping system table migration until there are any dead nodes.");
            co_return std::move(guard);
        }

        LOGMACRO(rtlogger, log_level::info, "Migrating view_builder to v2");
...
future<bool> topology_coordinator::maybe_start_tablet_migration(group0_guard guard) {
    LOGMACRO(rtlogger, log_level::debug, "Evaluating tablet balance");
    ...
    auto plan = co_await _tablet_allocator.balance_tablets(tm, {}, get_dead_nodes());
    if (plan.empty()) {
        LOGMACRO(rtlogger, log_level::debug, "Tablet load balancer did not make any plan");
        co_return false;
    }
...
future<> topology_coordinator::refresh_tablet_load_stats() {
    ...
    for (auto& [dc, nodes] : tm->get_datacenter_token_owners_nodes()) {
        ...
        LOGMACRO(rtlogger, log_level::debug, "raft topology: Refreshing table load stats for DC {} that has {} token owners", dc, nodes.size());
        co_await coroutine::parallel_for_each(nodes, [&] (const auto& node) -> future<> {
            ...
            if (!_gossiper.is_alive(dst)) {
                if (_load_stats_per_node.contains(dst)) {
                    ...
                    LOGMACRO(rtlogger, log_level::debug, "raft topology: Unable to refresh table load on {} because it's down.", dst);
        ...
        for (auto& [table_id, table_stats] : dc_stats.tables) {
            ...
            LOGMACRO(rtlogger, log_level::debug, "raft topology: Refreshed table load stats for DC {}, table={}, RF={}, size_in_bytes={}, split_ready_seq_number={}",
    ...
    LOGMACRO(rtlogger, log_level::debug, "raft topology: Refreshed table load stats for all DC(s).");
...
future<> topology_coordinator::start_tablet_load_stats_refresher() {
    ...
    while (can_proceed()) {
        ...
        try {
            ...
            LOGMACRO(rtlogger, log_level::debug, "raft topology: Tablet load stats refresher aborted");
            sleep = false;
        } catch (seastar::abort_requested_exception&) {
            LOGMACRO(rtlogger, log_level::debug, "raft topology: Tablet load stats refresher aborted");
            sleep = false;
        ...
        if (sleep && can_proceed()) {
            try {
                ...
                LOGMACRO(rtlogger, log_level::debug, "raft topology: Tablet load stats refresher: sleep failed: {}", std::current_exception());
            }
        }
    }
}
...
future<> topology_coordinator::build_coordinator_state(group0_guard guard) {
    ...
    LOGMACRO(rtlogger, log_level::info, "waiting for all nodes to finish upgrade to raft schema");
    release_guard(std::move(guard));
    ...
    if (!sl_version || *sl_version < 2) {
        LOGMACRO(rtlogger, log_level::info, "migrating service levels data");
    ...
    if (auth_version < db::system_keyspace::auth_version_t::v2) {
        LOGMACRO(rtlogger, log_level::info, "migrating system_auth keyspace data");
        co_await auth::migrate_to_auth_v2(_sys_ks, _group0.client(),
                [this] (abort_source&) { return start_operation();}, _as);
    }

    LOGMACRO(rtlogger, log_level::info, "building initial raft topology state and CDC generation");
    guard = co_await start_operation();
    ...
    for (const auto& node: tmptr->get_topology().get_nodes()) {
        ...
        LOGMACRO(rtlogger, log_level::debug, "node {} will contain the following parameters: "
...
future<> topology_coordinator::fence_previous_coordinator() {
    ...
    while (!_as.abort_requested()) {
        try {
            ...
            LOGMACRO(rtlogger, log_level::debug, "request to fence previous coordinator was aborted");
            break;
        } catch (...) {
        ...
        try {
            ...
            LOGMACRO(rtlogger, log_level::debug, "sleep failed while fencing previous coordinator: {}", std::current_exception());
        }
    }
}

future<> topology_coordinator::rollback_current_topology_op(group0_guard&& guard) {
    LOGMACRO(rtlogger, log_level::info, "start rolling back topology change");
...
bool topology_coordinator::handle_topology_coordinator_error(std::exception_ptr eptr) noexcept {
    try {
        ...
        LOGMACRO(rtlogger, log_level::debug, "topology change coordinator fiber aborted");
    } catch (seastar::abort_requested_exception&) {
        LOGMACRO(rtlogger, log_level::debug, "topology change coordinator fiber aborted");
    } catch (raft::commit_status_unknown&) {
        rtlogger.warn("topology change coordinator fiber got commit_status_unknown");
    } catch (group0_concurrent_modification&) {
        LOGMACRO(rtlogger, log_level::info, "topology change coordinator fiber got group0_concurrent_modification");
    } catch (topology_coordinator::term_changed_error&) {
        // Term changed. We may no longer be a leader
        LOGMACRO(rtlogger, log_level::debug, "topology change coordinator fiber notices term change {} -> {}", _term, _raft.get_current_term());
    } catch (...) {
...
future<bool> topology_coordinator::maybe_run_upgrade() {
    ...
    LOGMACRO(rtlogger, log_level::info, "topology coordinator fiber is upgrading the cluster to raft topology mode");
    ...
    while (!_as.abort_requested() && _topo_sm._topology.upgrade_state != topology::upgrade_state_type::done) {
        ...
        if (sleep) {
            try {
                ...
                LOGMACRO(rtlogger, log_level::debug, "sleep failed: {}", std::current_exception());
            }
        }
        co_await coroutine::maybe_yield();
    }
    ...
    while (!_as.abort_requested()) {
        ...
        try {
            ...
            if (!had_work) {
                ...
                LOGMACRO(rtlogger, log_level::debug, "topology coordinator fiber has nothing to do. Sleeping.");
                co_await await_event();
                LOGMACRO(rtlogger, log_level::debug, "topology coordinator fiber got an event");
            }
...
future<> topology_coordinator::stop() {
    ...
    co_await coroutine::parallel_for_each(_tablets, [] (auto& tablet) -> future<> {
        ...
        LOGMACRO(rtlogger, log_level::debug, "Checking tablet migration state for {}", gid);
...
future<> run_topology_coordinator(
    ...
    try {
        LOGMACRO(rtlogger, log_level::info, "start topology coordinator fiber");
