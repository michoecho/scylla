...
dictionary_service::dictionary_service(
    ...
    , _leadership_observer(raft_group0.observe_leadership([this] (bool leader) {
        utils::LOGMACRO(dict_trainer_logger, log_level::debug, "dictionary_service: _leadership_observer triggered");
        _is_leader = leader;
        maybe_toggle_dict_training();
    }))
    , _when_observer(_rpc_dict_training_when.observe([this] (const auto&) {
        utils::LOGMACRO(dict_trainer_logger, log_level::debug, "dictionary_service: _when_observer triggered");
        maybe_toggle_dict_training();
    }))
    , _feature_observer(fs.compression_dicts.when_enabled([
        ...
        utils::LOGMACRO(dict_trainer_logger, log_level::debug, "dictionary_service: _feature_observer triggered");
...
void dictionary_service::maybe_toggle_dict_training() {
    ...
    utils::LOGMACRO(dict_trainer_logger, log_level::debug, "dictionary_service::maybe_toggle_dict_training(), called, _is_leader={}, when={}", _is_leader, when);
...
future<> dictionary_service::stop() {
    utils::LOGMACRO(dict_trainer_logger, log_level::debug, "dictionary_service::stop(), called");
...
future<> dictionary_service::publish_dict(utils::dict_sampler::dict_type d) {
    while (true) {
        utils::LOGMACRO(dict_trainer_logger, log_level::debug, "dictionary_service::publish_dict(), called");
        try {
            utils::LOGMACRO(dict_trainer_logger, log_level::debug, "dictionary_service::publish_dict(), trying");
            ...
            utils::LOGMACRO(dict_trainer_logger, log_level::debug, "dictionary_service::publish_dict(), committing");
            co_await std::move(batch).commit(_raft_group0_client, _as, {});
            utils::LOGMACRO(dict_trainer_logger, log_level::debug, "dictionary_service::publish_dict(), finished");
            break;
        } catch (const service::group0_concurrent_modification&) {
            utils::LOGMACRO(dict_trainer_logger, log_level::debug, "group0_concurrent_modification in dictionary_service::publish_dict(), retrying");
        }
    }
}
