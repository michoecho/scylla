...
void encode_component(const abstract_type& type, managed_bytes_view serialized_bytes_view, bytes_ostream& out) {
    if (serialized_bytes_view.empty() && !has_encoding_for_zero_sized_value(type)) {
        ...
        // has no encoding of its own here: the element encodings are just concatenated, so an
        // element which encodes to nothing would be indistinguishable from a missing one.
        // It gets its own marker instead, and no value bytes at all.
        //
        // This deviates from Cassandra, which encodes an empty element exactly like a null one.
        // That makes Cassandra's encoding lossy -- an empty element and a null one both decode
        // back to null -- and it contradicts how the same distinction is drawn one level up,
        // where a clustering key component uses separate markers for the two. We use the
        // clustering key's markers here as well.
        //
        // (Note that unlike a trailing null element of a tuple, a trailing empty one isn't
        // omitted -- it goes through this path like any other.)
...
// deal with the "empty" values -- exactly the ones for which it returns null. Tuples
// are the one type where we draw the line differently than Cassandra does.
...
comparable_bytes comparable_bytes_from_compound(const compound_type<AllowPrefixes>& p, managed_bytes_view representation, std::byte terminator) {
    ...
    while (c_it != c_end) {
        ...
        if (mbv.empty() && !has_encoding_for_zero_sized_value(type)) {
            ...
            // It compares before all other values.
            //
