...
public:
    ...
    template <typename BytesViewIterator>
    static bytes_ostream pack_fragmented(BytesViewIterator start, BytesViewIterator finish, int elements, cql_serialization_format sf);
...
template <typename BytesViewIterator>
bytes_ostream
collection_type_impl::pack_fragmented(BytesViewIterator start, BytesViewIterator finish, int elements, cql_serialization_format sf) {
    size_t csl = collection_size_len(sf);
    size_t psz = collection_value_len(sf);

    size_t len = csl;
    for (auto j = start; j != finish; j++) {
        len += j->size_bytes() + psz;
    }
    ...
    while (start != finish) {
        write_collection_value(out, sf, *start++);
    }
    return out;
}
