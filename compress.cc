...
compression_parameters::compression_parameters(algorithm alg)
    : compression_parameters::compression_parameters(std::map<sstring, sstring>{
        {sstring(SSTABLE_COMPRESSION), sstring(algorithm_names[int(alg)])},
    })
{}

auto compression_parameters::name_to_algorithm(std::string_view name) -> algorithm {
    auto qn = sstring(qualified_name(compressor::namespace_prefix, name));
...
compression_parameters::compression_parameters(const std::map<sstring, sstring>& options) {
    ...
    if (auto v = get_option(SSTABLE_COMPRESSION)) {
        auto qn = sstring(qualified_name(compressor::namespace_prefix, *v));
        _algorithm = algorithm(std::ranges::find(algorithm_names, qn) - std::begin(algorithm_names));
    } else {
        _algorithm = algorithm::lz4;
    }
    ...
    if (chunk_length) {
        try {
            ...
            throw exceptions::syntax_exception(sstring("Invalid integer value ") + *chunk_length + " for " + *chunk_length);
        }
    }
