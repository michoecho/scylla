...
db::config::config(std::shared_ptr<db::extensions> exts)
    ...
    */
    ...
    , compressioninfo_cache_fraction(this, "compressioninfo_cache_fraction", liveness::LiveUpdate, value_status::Used, 0.1,
        "The maximum fraction of shard memory permitted for use by the cache of SSTable compression info (the chunk offsets of compressed ``Data.db`` files, stored in ``CompressionInfo.db``). Clamped to the [0.0; 1.0] range. Compression info is evicted only when it exceeds this fraction, so the value only has to be big enough to hold the working set of the reads; the memory isn't reserved and is usable by the row cache when the compression info doesn't need it.")
    , compressioninfo_is_evictable(this, "compressioninfo_is_evictable", liveness::MustRestart, value_status::Used, true,
        "If true, the chunk offsets of compressed SSTables are read from ``CompressionInfo.db`` on demand and cached evictably, so that the memory footprint of an open SSTable doesn't have to grow with the size of its data. If false, all chunk offsets of all open SSTables are kept in memory permanently, as they were before this option was introduced. This is an escape hatch, to be used if the evictable cache turns out to be problematic.")
