...
* Clustering Key Index (`Rows.db`)  
  ...
  (`da` in Cassandra, `ms`-`mu` in Scylla).
...
The versions currently supported by Scylla are: `ka`, `la`, `mc`, `md`, `me`, `ms`, `mt`, `mu`.
...
(`ms`-`mu` are Scylla-specific extensions of `me`, so they break away from the `mc`-`me` series).
...
* `<big>` is an attribute that identifies the SSTable sub-format.
  ...
  Versions `ms`-`mu` are a hybrid of `big` and `bti`).
