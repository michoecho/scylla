...
struct bti_node_reader {
    ...
    trie::node_traverse_result traverse_node_by_key(int64_t pos, const_bytes key) {
        ...
        case SINGLE_16:
            return single(sp[1], uint64_t(sp[1]) << 8 | uint64_t(sp[2]), uint8_t(sp[0]) & 0xf);
        case SPARSE_8:
            return sparse(type);
        case SPARSE_12:
            return sparse(type);
    ...
    trie::node_traverse_sidemost_result traverse_node_leftmost(int64_t pos) {
        ...
        case SINGLE_16:
            return single(uint64_t(sp[1]) << 8 | uint64_t(sp[2]), uint8_t(sp[0]) & 0xf);
        case SPARSE_8:
            return sparse(type);
        case SPARSE_12:
            return sparse(type);
    ...
    trie::get_child_result get_child(int64_t pos, int child_idx, bool forward) {
        ...
        case SINGLE_16:
            return single(uint64_t(sp[1]) << 8 | uint64_t(sp[2]));
        case SPARSE_8:
            return sparse(type);
        case SPARSE_12:
            return sparse(type);
