...
enum class sstable_version_types { ka, la, mc, md, me, ms, mt, mu };
...
// Whether the BTI index uses the legacy payload format (`ms`/`mt`), which stores
// a single logical position, as opposed to the `mu` format, which stores a full
// physical position (chunk coordinates + uncompressed offset).
