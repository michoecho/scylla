...
private:
    ...
    bti_trie_sink _twofw; 
    partition_trie_writer _pitw;
    bti_trie_sink _trwofw; 
    row_trie_writer _ritw;
...
void writer::init_file_writers() {
    ...
    if (_sst._schema->partition_key_type()->has_memcmp_comparable_form()) {
        _twofw = make_bti_trie_sink(*_trie_index_writer, 16*1024);
        ...
        if (_sst._schema->clustering_key_size() > 0 && _sst._schema->clustering_key_type()->has_memcmp_comparable_form()) {
            _trwofw = make_bti_trie_sink(*_trie_row_index_writer, 16*1024);
    ...
    if (_pitw) {
        ...
        if (_ritw) {
            if (auto root = _ritw.finish(); root >= 0) {
                ...
                // FIXME: don't reconstruct, just reset.
                _ritw = make_row_trie_writer(_trwofw);
            }
        }
