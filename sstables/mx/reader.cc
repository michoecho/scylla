...
public:
    ...
    data_consumer::proceed consume_partition_start(sstables::key_view key, sstables::deletion_time deltime) {
        ...
        auto proceed = _reader->on_next_partition(std::move(dk), tombstone(deltime));
        if (!bool(proceed)) {
            return data_consumer::proceed::no;
        }
        return data_consumer::proceed(!_reader->is_buffer_full() && !need_preempt());
...
public:
    ...
    // Advances the index to the first position to the first position
        ...
        return maybe_initialize().then([this, &pr] (bool initialized) {
            ...
            if (!initialized) {
                ...
                return f1.then([this] {
                    ...
                    if (start < _context->position()) {
                        sstlog.trace("mp_row_consumer_reader_mx {}: _paused_after_partition_start={}", fmt::ptr(this), _paused_after_partition_start);
                        ...
                        // (This is signified by _paused_after_partition_start).
    ...
    data_consumer::proceed on_next_partition(dht::decorated_key key, tombstone tomb) override {
        if (_pr.get().before(key, dht::ring_position_comparator(*_schema))) {
            ...
            // to handle that case.
            _paused_after_partition_start = true;
            _current_partition_key = std::move(key);
...
public:
    ...
    data_consumer::proceed consume_partition_start(sstables::key_view key, sstables::deletion_time deltime) {
        auto _last_pkey = key.to_partition_key(*_schema);
        ...
        while (!context->eof() && !abort.abort_requested()) {
            ...
            if (idx_reader) {
                ...
                if (pk) {
                    ...
                    sstlog.trace("validate(): index-data position check for a partition: {} == {}", *pk, data_pos, index_pos);
                }
