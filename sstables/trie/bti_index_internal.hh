...
struct row_index_header {
    ...
    uint64_t trie_root;
    // The position of the partition inside Data.db.
    //
    // For `ms`/`mt` this is a logical (uncompressed) position; for `mu` it carries
    // the full chunk coordinates locating the partition start in the compressed
    // (on-disk) file, so that the physical cursor can navigate to it directly.
    sstables::sstable_datafile_position data_file_position;
