...
future<sstables::shared_sstable> sstable_directory::load_sstable(sstables::entry_descriptor desc,
    ...
    shared_sstable sst = _manager.make_sstable(_schema, storage_opts, desc.generation, _state, desc.version, gc_clock::now(), _error_handler_gen);
...
future<shared_sstable> sstable_directory::load_foreign_sstable(foreign_sstable_open_info& info) {
    auto sst = _manager.make_sstable(_schema, *_storage_opts, info.generation, _state, info.version, gc_clock::now(), _error_handler_gen);
...
future<std::vector<shard_id>> sstable_directory::get_shards_for_this_sstable(
    ...
    auto sst = _manager.make_sstable(_schema, storage_opts, desc.generation, _state, desc.version, gc_clock::now(), _error_handler_gen);
