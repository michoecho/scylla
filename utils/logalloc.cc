...
void allocating_section::on_alloc_failure(logalloc::region& r) {
    ...
    if (r.get_tracker().get_impl().segment_pool().allocation_failure_flag()) {
        _lsa_reserve *= 2;
        LOGMACRO(llogger, log_level::info, "LSA allocation failure, increasing reserve in section {} to {} segments; trace: {}", fmt::ptr(this), _lsa_reserve, current_backtrace());
    } else {
        _std_reserve *= 2;
        LOGMACRO(llogger, log_level::info, "Standard allocator failure, increasing head-room in section {} to {} [B]; trace: {}", fmt::ptr(this), _std_reserve, current_backtrace());
    }
