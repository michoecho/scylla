 ...
 * live in tracing/tracer.cc, behind the out-of-line hooks declared
 * below. The one exception is run_task: it fires in the reactor's hottest loop,
 * where its static-ID record is emitted directly at the call site.
 *
...
// The events below are one TRACEPOINT() each in tracer.cc; see the file
// comment there for what the trace viewer makes of them. run_task is the
...
void trace_io_begin(uint32_t task, uint64_t io) noexcept;
void trace_io_end(uint32_t task, uint64_t io) noexcept;
