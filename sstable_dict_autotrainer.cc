...
future<> sstable_dict_autotrainer::tick() {
    LOGMACRO(alogger, log_level::debug, "sstable_dict_autotrainer::tick(): entering");
    ...
    _ss.get_database().get_tables_metadata().for_each_table([&dict_aware_tables] (table_id id, lw_shared_ptr<replica::table> table) {
        ...
        if (compression_algo == compressor::algorithm::lz4_with_dicts || compression_algo == compressor::algorithm::zstd_with_dicts) {
            LOGMACRO(alogger, log_level::debug, "sstable_dict_autotrainer::tick(): {}.{} is dict-aware", table->schema()->ks_name(), table->schema()->cf_name());
            dict_aware_tables.push_back(table->schema());
        } else {
            LOGMACRO(alogger, log_level::debug, "sstable_dict_autotrainer::tick(): {}.{} is not dict-aware", table->schema()->ks_name(), table->schema()->cf_name());
        }
    });
    for (const auto& s : dict_aware_tables) {
        ...
        if (current_dict_timestamp) {
            LOGMACRO(alogger, log_level::debug, "sstable_dict_autotrainer::tick(): {}.{} has timestamp {}", s->ks_name(), s->cf_name(), current_dict_timestamp);
            auto delta = (db_clock::now() - *current_dict_timestamp);
            auto min_delta = std::chrono::duration<float>(_cfg.retrain_period_in_seconds());
            if (delta < min_delta) {
                LOGMACRO(alogger, log_level::debug, "sstable_dict_autotrainer::tick(): {}.{} has dict younger than {}, not updating", s->ks_name(), s->cf_name(), min_delta);
                continue;
            }
        } else {
            LOGMACRO(alogger, log_level::debug, "sstable_dict_autotrainer::tick(): {}.{} has no dict", s->ks_name(), s->cf_name());
        }
        LOGMACRO(alogger, log_level::debug, "sstable_dict_autotrainer::tick(): attempting to update the dict for {}.{}", s->ks_name(), s->cf_name());
        auto estimated_size = co_await _ss.estimate_total_sstable_volume(s->id());
        LOGMACRO(alogger, log_level::debug, "sstable_dict_autotrainer::tick(): {}.{}: estimated_size={}", s->ks_name(), s->cf_name(), estimated_size);
        auto min_dataset_size = _cfg.min_dataset_bytes();
        if (estimated_size < min_dataset_size) {
            LOGMACRO(alogger, log_level::debug, "sstable_dict_autotrainer::tick(): {}.{}: dataset not big enough yet (< {}), giving up on the update", s->ks_name(), s->cf_name(), min_dataset_size);
            continue;
        }
        ...
        LOGMACRO(alogger, log_level::debug, "sstable_dict_autotrainer::tick(): {}.{}: sampling for training with chunk_length={}, n_chunks={}", s->ks_name(), s->cf_name(), params.chunk_length(), n_chunks);
        auto training_sample = co_await _ss.do_sample_sstables(s->id(), params.chunk_length(), n_chunks);
        LOGMACRO(alogger, log_level::debug, "sstable_dict_autotrainer::tick(): {}.{}: got training sample with {} chunks", s->ks_name(), s->cf_name(), training_sample.size());
        if (training_sample.size() < n_chunks) {
            LOGMACRO(alogger, log_level::debug, "sstable_dict_autotrainer::tick(): {}.{}: not enough chunks, giving up on the update", s->ks_name(), s->cf_name());
            continue;
        }
        auto dict = co_await _ss.train_dict(std::move(training_sample));
        LOGMACRO(alogger, log_level::debug, "sstable_dict_autotrainer::tick(): {}.{}: trained dict of size {}", s->ks_name(), s->cf_name(), dict.size());
        LOGMACRO(alogger, log_level::debug, "sstable_dict_autotrainer::tick(): {}.{}: sampling for validation with chunk_length={}, n_chunks={}", s->ks_name(), s->cf_name(), params.chunk_length(), n_chunks);
        auto validation_sample = co_await _ss.do_sample_sstables(s->id(), params.chunk_length(), n_chunks);
        LOGMACRO(alogger, log_level::debug, "sstable_dict_autotrainer::tick(): {}.{}: got validation sample with {} chunks", s->ks_name(), s->cf_name(), validation_sample.size());
        ...
        LOGMACRO(alogger, log_level::debug, "sstable_dict_autotrainer::tick(): {}.{}: ratio_before={}, ratio_after={}, update_threshold={}", s->ks_name(), s->cf_name(), ratio_before, ratio_after, threshold);
        if (ratio_after > threshold) {
            LOGMACRO(alogger, log_level::debug, "sstable_dict_autotrainer::tick(): {}.{}: ratio_after below threshold. Discarding the new dict.", s->ks_name(), s->cf_name());
            continue;
        }
        LOGMACRO(alogger, log_level::debug, "sstable_dict_autotrainer::tick(): {}.{}: Publishing the new dict.", s->ks_name(), s->cf_name());
...
future<> sstable_dict_autotrainer::run() {
    ...
    LOGMACRO(alogger, log_level::debug, "sstable_dict_autotrainer::run(): starting");
    ...
    while (!_as.abort_requested()) {
        try {
            ...
            LOGMACRO(alogger, log_level::debug, "sstable_dict_autotrainer::run(): exiting");
        } catch (...) {
            LOGMACRO(alogger, log_level::debug, "sstable_dict_autotrainer::run(): tick() failed with: {}", std::current_exception());
        }
    }
}
