...
future<> gossiper::handle_syn_msg(locator::host_id from, gossip_digest_syn syn_msg) {
    LOGMACRO(logger, log_level::trace, "handle_syn_msg():from={},cluster_name:peer={},local={},group0_id:peer={},local={},partitioner_name:peer={},local={}",
    ...
    if (p.pending) {
        ...
        // one only.
        LOGMACRO(logger, log_level::debug, "Queue gossip syn msg from node {}, syn_msg={}", from, syn_msg);
        p.syn_msg = std::move(syn_msg);
        co_return;
    }

    // Process the syn message immediately
    LOGMACRO(logger, log_level::debug, "Process gossip syn msg from node {}, syn_msg={}", from, syn_msg);
    p.pending = true;
    for (;;) {
        try {
            ...
            if (p.syn_msg) {
                ...
                LOGMACRO(logger, log_level::debug, "Handle queued gossip syn msg from node {}, syn_msg={}, pending={}",
                ...
                p.pending = false;
                LOGMACRO(logger, log_level::debug, "No more queued gossip syn msg from node {}, syn_msg={}, pending={}",
                        from, p.syn_msg, p.pending);
                co_return;
            }
        } catch (...) {
...
future<> gossiper::do_send_ack_msg(locator::host_id from, gossip_digest_syn syn_msg) {
    ...
    LOGMACRO(logger, log_level::debug, "Calling do_send_ack_msg to node {}, syn_msg={}, ack_msg={}", from, syn_msg, ack_msg);
    ...
    for (const auto& x : map) {
        ...
        for (const auto& entry : state.get_application_state_map()) {
            ...
            if (is_critical_state(app_state)) {
                ...
                LOGMACRO(logger, log_level::debug, "node={}, app_state={}, count_as_msg_processing={}",
...
future<> gossiper::handle_ack_msg(locator::host_id id, gossip_digest_ack ack_msg) {
    LOGMACRO(logger, log_level::trace, "handle_ack_msg():from={},msg={}", id, ack_msg);
    ...
    if (p.pending) {
        ...
        // one only.
        LOGMACRO(logger, log_level::debug, "Queue gossip ack msg digests from node {}, ack_msg_digest={}", from, ack_msg_digest);
        p.ack_msg_digest = std::move(ack_msg_digest);
        co_return;
    }

    // Process the ack message immediately
    LOGMACRO(logger, log_level::debug, "Process gossip ack msg digests from node {}, ack_msg_digest={}", from, ack_msg_digest);
    p.pending = true;
    for (;;) {
        try {
            ...
            if (p.ack_msg_digest) {
                ...
                LOGMACRO(logger, log_level::debug, "Handle queued gossip ack msg digests from node {}, ack_msg_digest={}, pending={}",
                ...
                p.pending = false;
                LOGMACRO(logger, log_level::debug, "No more queued gossip ack msg digests from node {}, ack_msg_digest={}, pending={}",
                        from, p.ack_msg_digest, p.pending);
                co_return;
            }
        } catch (...) {
...
future<> gossiper::do_send_ack2_msg(locator::host_id from, utils::chunked_vector<gossip_digest> ack_msg_digest) {
    ...
    LOGMACRO(logger, log_level::debug, "Calling do_send_ack2_msg to node {}, ack_msg_digest={}, ack2_msg={}", from, ack_msg_digest, ack2_msg);
    co_await ser::gossip_rpc_verbs::send_gossip_digest_ack2(&_messaging, from, std::move(ack2_msg));
    LOGMACRO(logger, log_level::debug, "finished do_send_ack2_msg to node {}, ack_msg_digest={}, ack2_msg={}", from, ack_msg_digest, ack2_msg);
}
...
future<> gossiper::handle_ack2_msg(locator::host_id from, gossip_digest_ack2 msg) {
    LOGMACRO(logger, log_level::trace, "handle_ack2_msg():msg={}", msg);
    if (!is_enabled()) {
        co_return;
    }
    ...
    if (!_advertise_to_nodes.empty()) {
        ...
        if (it == _advertise_to_nodes.end()) {
            ...
            if (es) {
                ...
                LOGMACRO(logger, log_level::debug, "handle_echo_msg: from={}, saved_generation_number={}, current_generation_number={}",
...
future<> gossiper::handle_shutdown_msg(locator::host_id from, std::optional<int64_t> generation_number_opt) {
    if (!is_enabled()) {
        LOGMACRO(logger, log_level::debug, "Ignoring shutdown message from {} because gossip is disabled", from);
        co_return;
    }
    ...
    if (generation_number_opt) {
        ...
        if (es) {
            ...
            LOGMACRO(logger, log_level::info, "Got shutdown message from {}, received_generation={}, local_generation={}",
...
future<> gossiper::send_gossip(gossip_digest_syn message, std::set<T> epset) {
    ...
    LOGMACRO(logger, log_level::trace, "Sending a GossipDigestSyn to {} ...", id);
    return ser::gossip_rpc_verbs::send_gossip_digest_syn(&_messaging, id, std::move(message)).handle_exception([id] (auto ep) {
        ...
        LOGMACRO(logger, log_level::trace, "Fail to send GossipDigestSyn to {}: {}", id, ep);
    });
}
...
future<> gossiper::do_apply_state_locally(locator::host_id node, endpoint_state remote_state, bool shadow_round) {
    ...
    if (es) {
        ...
        LOGMACRO(logger, log_level::trace, "{} local generation {}, remote generation {}", node, local_generation, remote_generation);
        if (remote_generation > generation_type(get_generation_number().value() + MAX_GENERATION_DIFFERENCE)) {
            ...
            LOGMACRO(logger, log_level::trace, "Updating heartbeat state generation to {} from {} for {} (notify={})", remote_generation, local_generation, node, !shadow_round);
            ...
            if (remote_max_version > local_max_version) {
                ...
                LOGMACRO(logger, log_level::debug, "Ignoring remote version {} <= {} for {}", remote_max_version, local_max_version, node);
            }
            ...
            LOGMACRO(logger, log_level::debug, "Ignoring remote generation {} < {}", remote_generation, local_generation);
        }
    } else {
        LOGMACRO(logger, log_level::debug, "Applying remote_state for node {} ({} node)", node, !shadow_round ? "old" : "new");
        co_await handle_major_state_change(std::move(remote_state), permit.id(), shadow_round);
    }
}
...
future<> gossiper::apply_state_locally(std::map<inet_address, endpoint_state> map) {
    ...
    LOGMACRO(logger, log_level::debug, "apply_state_locally_endpoints={}", endpoints);
    ...
    co_await coroutine::parallel_for_each(endpoints, [this, &map] (auto&& ep) -> future<> {
        ...
        if (hid == my_host_id()) {
            LOGMACRO(logger, log_level::trace, "Ignoring gossip for {} because it maps to local id, but is not local address", ep);
            return make_ready_future<>();
        }
        if (_topo_sm) {
            if (_topo_sm->_topology.left_nodes.contains(raft::server_id(hid.uuid()))) {
                LOGMACRO(logger, log_level::trace, "Ignoring gossip for {} because it left", ep);
                return make_ready_future<>();
            }
        } else {
            if (_just_removed_endpoints.contains(hid)) {
                LOGMACRO(logger, log_level::trace, "Ignoring gossip for {} because it is quarantined", hid);
                return make_ready_future<>();
            }
        }
    ...
    LOGMACRO(logger, log_level::debug, "apply_state_locally() took {} ms", std::chrono::duration_cast<std::chrono::milliseconds>(
    ...
    return container().invoke_on(0, [this, pid, id] (auto& gossiper) mutable -> future<> {
        ...
        try {
            ...
            if (!gossiper._endpoint_state_map.contains(id)) {
                LOGMACRO(logger, log_level::debug, "Force remove node is called on non exiting endpoint {}", id);
                co_return;
            }
            co_await gossiper.remove_endpoint(id, pid);
            co_await gossiper.evict_from_membership(id, pid);
            LOGMACRO(logger, log_level::info, "Finished to force remove node {}", id);
        } catch (...) {
...
future<> gossiper::remove_endpoint(locator::host_id endpoint, permit_id pid) {
    ...
    if(_seeds.contains(ip)) {
        build_seeds_list();
        _seeds.erase(ip);
        LOGMACRO(logger, log_level::info, "removed {} from _seeds, updated _seeds list = {}", endpoint, _seeds);
    }
    ...
    LOGMACRO(logger, log_level::info, "Removed endpoint {}", endpoint);

    if (was_alive) {
        try {
            LOGMACRO(logger, log_level::info, "InetAddress {}/{} is now DOWN, status = {}", state->get_host_id(), endpoint, get_gossip_status(*state));
...
future<> gossiper::do_status_check() {
    LOGMACRO(logger, log_level::trace, "Performing status check ...");
    ...
    for (const auto& host_id : get_endpoints()) {
        ...
        if (is_gossip_only_member(host_id)
            ...
            LOGMACRO(logger, log_level::info, "FatClient {} has been silent for {}ms, removing from gossip", host_id, fat_client_timeout.count());
        ...
        if (!is_alive && (now > expire_time)) {
            ...
            if (!host_id || !node || !node->is_member()) {
                LOGMACRO(logger, log_level::debug, "time is expiring for endpoint : {} ({})", host_id, expire_time.time_since_epoch().count());
    ...
    for (auto it = _just_removed_endpoints.begin(); it != _just_removed_endpoints.end();) {
        ...
        if ((now - t) > quarantine_delay()) {
            LOGMACRO(logger, log_level::info, "{} ms elapsed, {} gossip quarantine over", quarantine_delay().count(), it->first);
    ...
    , _caller(std::move(caller))
{
    ...
    LOGMACRO(logger, log_level::debug, "{}: lock_endpoint {}: acquired: permit_id={} holders={}", _caller.function_name(), _addr, _permit_id, _ptr->holders);
}
...
bool gossiper::endpoint_permit::release() noexcept {
    if (auto ptr = std::exchange(_ptr, nullptr)) {
        ...
        LOGMACRO(logger, log_level::debug, "{}: lock_endpoint {}: released: permit_id={} holders={}", _caller.function_name(), _addr, _permit_id, ptr->holders);
        if (!--ptr->holders) {
            LOGMACRO(logger, log_level::debug, "{}: lock_endpoint {}: released: permit_id={}", _caller.function_name(), _addr, _permit_id);
...
future<gossiper::endpoint_permit> gossiper::lock_endpoint(locator::host_id ep, permit_id pid, seastar::compat::source_location l) {
    ...
    LOGMACRO(logger, log_level::debug, "{}: lock_endpoint {}: waiting: permit_id={}", l.function_name(), ep, pid);
    while (true) {
        _abort_source.check();
...
future<> gossiper::failure_detector_loop_for_node(locator::host_id host_id, generation_type gossip_generation, uint64_t live_endpoints_version) {
    ...
    while (is_enabled() && !_abort_source.abort_requested()) {
        bool failed = false;
        try {
            LOGMACRO(logger, log_level::debug, "failure_detector_loop: Send echo to node {}/{}, status = started", host_id, node);
            co_await send_echo(host_id, max_duration, gossip_generation.value(), false);
            LOGMACRO(logger, log_level::debug, "failure_detector_loop: Send echo to node {}/{}, status = ok", host_id, node);
        } catch (...) {
            failed = true;
        ...
        if (diff > max_duration) {
            LOGMACRO(logger, log_level::info, "failure_detector_loop: Mark node {}/{} as DOWN", host_id, node);
        ...
        if (_live_endpoints_version != live_endpoints_version) {
            LOGMACRO(logger, log_level::debug, "failure_detector_loop: Finished loop for node {}/{}, live_endpoints={}, current_live_endpoints_version={}, live_endpoints_version={}",
...
future<> gossiper::failure_detector_loop() {
    ...
    LOGMACRO(logger, log_level::info, "failure_detector_loop: Started main loop");
    while (is_enabled()) {
        try {
            if (_live_endpoints.empty()) {
                LOGMACRO(logger, log_level::debug, "failure_detector_loop: Wait until live_nodes={} is not empty", _live_endpoints);
            ...
            co_await coroutine::parallel_for_each(std::views::iota(0u, nodes.size()), [this, generation_number, live_endpoints_version, &nodes] (size_t idx) {
                ...
                LOGMACRO(logger, log_level::debug, "failure_detector_loop: Started new round for node={} on shard={}, live_nodes={}, live_endpoints_version={}",
    ...
    LOGMACRO(logger, log_level::info, "failure_detector_loop: Finished main loop");
}
...
future<> gossiper::replicate_live_endpoints_on_change(foreign_ptr<std::unique_ptr<live_and_unreachable_endpoints>> data0, uint64_t new_version) {
    ...
    //
    LOGMACRO(logger, log_level::debug, "replicating live and unreachable endpoints to other shards");
...
void gossiper::run() {
  ...
  (void)seastar::with_semaphore(_callback_running, 1, [this] {
    return seastar::async([this, g = shared_from_this()] {
            LOGMACRO(logger, log_level::trace, "=== Gossip round START");
...

            {
                ...
                LOGMACRO(logger, log_level::trace, "My heartbeat is now {}", hbs.get_heart_beat_version());
            }
            ...
            if (g_digests.size() > 0) {
                ...
                if (_endpoints_to_talk_with.empty() && !_live_endpoints.empty()) {
                    ...
                    LOGMACRO(logger, log_level::debug, "Set live nodes to talk: endpoint_state_map={}, all_live_nodes={}, endpoints_to_talk_with={}",
                ...
                if (!_endpoints_to_talk_with.empty()) {
                    ...
                    LOGMACRO(logger, log_level::debug, "Talk to live nodes: {}", live_nodes);
                    for (auto& ep: live_nodes) {
                        (void)with_gate(_background_msg, [this, message, ep] () mutable {
                            return do_gossip_to_live_member(message, ep).handle_exception([] (auto ep) {
                                LOGMACRO(logger, log_level::trace, "Failed to send gossip to live members: {}", ep);
                            });
                        });
                    }
                } else if (!_seeds.empty()) {
                    LOGMACRO(logger, log_level::debug, "No live nodes yet: try initial contact point nodes={}", _seeds);
                    for (auto& ep: _seeds) {
                        (void)with_gate(_background_msg, [this, message, ep] () mutable {
                            return do_gossip_to_live_member(message, ep).handle_exception([] (auto ep) {
                                LOGMACRO(logger, log_level::trace, "Failed to send gossip to live members: {}", ep);
                    ...
                    LOGMACRO(logger, log_level::debug, "No one to talk with");
                }
                ...
                (void)with_gate(_background_msg, [this, message = std::move(message)] () mutable {
                    return do_gossip_to_unreachable_member(std::move(message)).handle_exception([] (auto ep) {
                        LOGMACRO(logger, log_level::trace, "Failed to send gossip to unreachable members: {}", ep);
                    });
                });
                if (!_topo_sm) {
                    do_status_check().get();
        ...
        try {
            f.get();
            _nr_run++;
            LOGMACRO(logger, log_level::trace, "=== Gossip round OK");
        } catch (...) {
        ...
        if (logger.is_enabled(logging::log_level::trace)) {
            for (auto& x : _endpoint_state_map) {
                LOGMACRO(logger, log_level::trace, "ep={}, eps={}", x.first, *x.second);
            }
        }
        if (is_enabled()) {
            _scheduled_gossip_task.arm(INTERVAL);
        } else {
            LOGMACRO(logger, log_level::info, "Gossip loop is not scheduled because it is disabled");
        }
    });
  });
}
...
std::set<locator::host_id> gossiper::get_live_members() const {
    ...
    LOGMACRO(logger, log_level::debug, "live_members before={}", live_members);
    if (!is_shutdown(myip)) {
        live_members.insert(my_host_id());
    }
    LOGMACRO(logger, log_level::debug, "live_members after={}", live_members);
    return live_members;
}
...
future<> gossiper::evict_from_membership(locator::host_id hid, permit_id pid) {
    ...
    LOGMACRO(logger, log_level::debug, "evicting {} from gossip", hid);
}
...
future<> gossiper::advertise_token_removed(locator::host_id host_id, permit_id pid) {
    ...
    LOGMACRO(logger, log_level::info, "Completing removal of {}", host_id);
...
future<> gossiper::assassinate_endpoint(sstring address) {
    ...
    co_await container().invoke_on(0, [&] (auto&& gossiper) -> future<> {
        ...
        LOGMACRO(logger, log_level::info, "Sleeping for {} ms to ensure {} does not change", ring_delay.count(), endpoint);
...
future<> gossiper::do_gossip_to_unreachable_member(gossip_digest_syn message) {
    ...
    if (unreachable_endpoint_count > 0) {
        ...
        if (rand_dbl < prob) {
            ...
            LOGMACRO(logger, log_level::trace, "do_gossip_to_unreachable_member: live_endpoint nr={} unreachable_endpoints nr={}",
...
future<> gossiper::reset_endpoint_state_map() {
    LOGMACRO(logger, log_level::debug, "Resetting endpoint state map");
    auto lock = co_await lock_endpoint_update_semaphore();
...
bool gossiper::is_cql_ready(const locator::host_id& endpoint) const {
    ...
    if (!eps) {
        LOGMACRO(logger, log_level::debug, "Node {} does not have RPC_READY application_state, return is_cql_ready=true", endpoint);
        return true;
    }
    auto ready = eps->is_cql_ready();
    LOGMACRO(logger, log_level::debug, "Node {}: is_cql_ready={}",  endpoint, ready);
    return ready;
}
...
std::optional<endpoint_state> gossiper::get_state_for_version_bigger_than(locator::host_id for_endpoint, version_type version) const {
    ...
    if (es) {
        ...
        if (local_hb_version > version) {
            ...
            LOGMACRO(logger, log_level::trace, "local heartbeat version {} greater than {} for {}", local_hb_version, version, for_endpoint);
        }
        ...
        for (auto& entry : eps.get_application_state_map()) {
            ...
            if (value.version() > version) {
                ...
                LOGMACRO(logger, log_level::trace, "Adding state of {}, {}: {}" , for_endpoint, key, value.value());
...
void gossiper::update_timestamp_for_nodes(const std::map<inet_address, endpoint_state>& map) {
    for (const auto& x : map) {
        ...
        if (local_endpoint_state) {
            ...
            if (update) {
                LOGMACRO(logger, log_level::trace, "Updated timestamp for node {}", endpoint);
...
void gossiper::mark_alive(endpoint_state_ptr node) {
    ...
    if (inserted) {
        ...
        LOGMACRO(logger, log_level::debug, "Mark Node {}/{} alive with EchoMessage", id, addr);
    } else {
        // We are in the progress of marking this node alive
        LOGMACRO(logger, log_level::debug, "Node {}/{} is being marked as up, ignoring duplicated mark alive operation", id, addr);
        return;
    }
    ...
    LOGMACRO(logger, log_level::debug, "Sending a EchoMessage to {}/{}, with generation_number={}", id, addr, generation);
    (void) send_echo(id, std::chrono::seconds(15), generation.value(), false).then([this, id] {
        LOGMACRO(logger, log_level::trace, "Got EchoMessage Reply");
...
future<> gossiper::real_mark_alive(locator::host_id host_id) {
    ...
    auto es = get_endpoint_state_ptr(host_id);
    if (!es) {
        LOGMACRO(logger, log_level::info, "Node {} is not in endpoint_state_map anymore", host_id);
        co_return;
    }
    ...
    LOGMACRO(logger, log_level::debug, "Mark Node {} alive after EchoMessage", host_id);

    // prevents do_status_check from racing us and evicting if it was down > A_VERY_LONG_TIME
    update_timestamp(es);

    LOGMACRO(logger, log_level::debug, "removing expire time for endpoint : {}", host_id);
    bool was_live = false;
    ...
    LOGMACRO(logger, log_level::info, "InetAddress {}/{} is now UP, status = {}", host_id, addr, status);
    ...
    co_await _subscribers.for_each([addr, host_id, es, pid = permit.id()] (shared_ptr<i_endpoint_state_change_subscriber> subscriber) -> future<> {
        ...
        LOGMACRO(logger, log_level::trace, "Notified {}", fmt::ptr(subscriber.get()));
    });
}

future<> gossiper::mark_dead(locator::host_id addr, endpoint_state_ptr state, permit_id pid) {
    LOGMACRO(logger, log_level::trace, "marking as down {}", addr);
    ...
    LOGMACRO(logger, log_level::info, "InetAddress {} is now DOWN, status = {}", addr, get_gossip_status(*state));
...
future<> gossiper::handle_major_state_change(endpoint_state eps, permit_id pid, bool shadow_round) {
    ...
    if (!is_dead_state(eps) && !shadow_round) {
        if (_endpoint_state_map.contains(ep))  {
            LOGMACRO(logger, log_level::info, "Node {} has restarted, now UP, status = {}", ep, get_gossip_status(eps));
        } else {
            LOGMACRO(logger, log_level::debug, "Node {} is now part of the cluster, status = {}", ep, get_gossip_status(eps));
        }
    }
    LOGMACRO(logger, log_level::trace, "Adding endpoint state for {}, status = {}", ep, get_gossip_status(eps));
    ...
    if (!is_dead_state(*ep_state)) {
        ...
        LOGMACRO(logger, log_level::debug, "Not marking {} alive due to dead state {}", ep, get_gossip_status(eps));
...
future<> gossiper::apply_new_states(endpoint_state local_state, const endpoint_state& remote_state, permit_id pid, bool shadow_round) {
    ...
    //     LOGMACRO(logger, log_level::trace, "Updating heartbeat state version to {} from {} for {} ...",
...
void gossiper::request_all(gossip_digest& g_digest,
    ...
    LOGMACRO(logger, log_level::trace, "request_all for {}", g_digest.get_endpoint());
}
...
void gossiper::send_all(gossip_digest& g_digest,
    ...
    LOGMACRO(logger, log_level::trace, "send_all(): ep={}/{}, version > {}", id, ep, max_remote_version);
    if (id) {
...
void gossiper::examine_gossiper(utils::chunked_vector<gossip_digest>& g_digest_list,
    ...
    for (gossip_digest& g_digest : g_digest_list) {
        ...
        if (es) {
            ...
            LOGMACRO(logger, log_level::trace, "examine_gossiper(): ep={}, remote={}.{}, local={}.{}", ep,
            ...
            if (remote_generation > local_generation) {
                ...
                if (max_remote_version > max_local_version) {
                    LOGMACRO(logger, log_level::trace, "examine_gossiper(): requesting version > {} from {}", max_local_version, g_digest.get_endpoint());
...
future<> gossiper::start_gossiping(gms::generation_type generation_nbr, application_state_map preload_local_states) {
    ...
    LOGMACRO(logger, log_level::info, "Gossip started with local state: {}", local_state);
    _enabled = true;
    _nr_run = 0;
    _scheduled_gossip_task.arm(INTERVAL);
...
future<> gossiper::do_shadow_round(std::unordered_set<gms::inet_address> nodes, mandatory is_mandatory) {
    ...
    LOGMACRO(logger, log_level::info, "Gossip shadow round started with nodes={}", nodes);
    ...
    for (;;) {
        ...
        co_await coroutine::parallel_for_each(nodes, [this, &request, &responses, &nodes_talked, &nodes_down] (gms::inet_address node) -> future<> {
            LOGMACRO(logger, log_level::debug, "Sent get_endpoint_states request to {}, request={}", node, request.application_states);
            try {
                ...
                LOGMACRO(logger, log_level::debug, "Got get_endpoint_states response from {}, response={}", node, response.endpoint_state_map);
        ...
        LOGMACRO(logger, log_level::info, "Connect nodes={} again ... ({} seconds passed)",
    ...
    LOGMACRO(logger, log_level::info, "Gossip shadow round finished with nodes_talked={}", nodes_talked);
}
...
future<> gossiper::add_saved_endpoint(locator::host_id host_id, gms::loaded_endpoint_state st, permit_id pid) {
    if (host_id == my_host_id()) {
        LOGMACRO(logger, log_level::debug, "Attempt to add self as saved endpoint");
        co_return;
    }
    ...
    if (es) {
        ...
        ep_state = *es;
        LOGMACRO(logger, log_level::debug, "not replacing a previous ep_state for {}, but reusing it: {}", ep, ep_state);
    ...
    LOGMACRO(logger, log_level::trace, "Adding saved endpoint {} {}", ep, generation);
}
...
future<> gossiper::do_stop_gossiping() {
    ...
    if (!_enabled) {
        LOGMACRO(logger, log_level::info, "gossip is already stopped");
        co_return;
    }
    auto my_ep_state = get_this_endpoint_state_ptr();
    if (my_ep_state) {
        LOGMACRO(logger, log_level::info, "My status = {}", get_gossip_status(*my_ep_state));
    }
    if (my_ep_state && !is_silent_shutdown_state(*my_ep_state)) {
        auto local_generation = my_ep_state->get_heart_beat_state().get_generation();
        LOGMACRO(logger, log_level::info, "Announcing shutdown");
        ...
        for (locator::host_id id : live_endpoints) {
            LOGMACRO(logger, log_level::info, "Sending a GossipShutdown to {} with generation {}", id, local_generation);
            try {
                co_await ser::gossip_rpc_verbs::send_gossip_shutdown(&_messaging, id, get_broadcast_address(), local_generation.value());
                LOGMACRO(logger, log_level::trace, "Got GossipShutdown Reply");
            } catch (...) {
    ...
    LOGMACRO(logger, log_level::info, "Disable and wait for gossip loop started");
    ...
    LOGMACRO(logger, log_level::info, "Gossip is now stopped");
}
...
void gossiper::add_expire_time_for_endpoint(locator::host_id endpoint, clk::time_point expire_time) {
    ...
    LOGMACRO(logger, log_level::info, "Node {} will be removed from gossip at [{:%Y-%m-%d %T}]: (expire = {}, now = {}, diff = {} seconds)",
    ...
    for (;;) {
        ...
        for (const auto& node: nodes) {
            ...
            if (es) {
                ...
                LOGMACRO(logger, log_level::debug, "Marked node={} as alive on {} out of {} shards", node, nr_alive, smp::count);
                if (nr_alive == smp::count) {
                    live_nodes.push_back(node);
                }
            }
        }
        LOGMACRO(logger, log_level::debug, "Waited for marking node as up, replace_nodes={}, live_nodes={}", nodes, live_nodes);
...
future<> gossiper::wait_for_live_nodes_to_show_up(size_t n) {
    ...
    LOGMACRO(logger, log_level::info, "Live nodes seen in gossip: {}", get_live_members());
}
...
future<> gossiper::wait_for_gossip(std::chrono::milliseconds initial_delay, std::optional<int32_t> force_after) const {
    ...
    while (num_okay < GOSSIP_SETTLE_POLL_SUCCESSES_REQUIRED) {
        ...
        if (current_size == ep_size && _msg_processing == 0) {
            LOGMACRO(logger, log_level::debug, "Gossip looks settled");
            num_okay++;
        } else {
            LOGMACRO(logger, log_level::info, "Gossip not settled after {} polls.", total_polls);
            num_okay = 0;
        }
    ...
    if (total_polls > GOSSIP_SETTLE_POLL_SUCCESSES_REQUIRED) {
        LOGMACRO(logger, log_level::info, "Gossip settled after {} extra polls; proceeding", total_polls - GOSSIP_SETTLE_POLL_SUCCESSES_REQUIRED);
    } else {
        LOGMACRO(logger, log_level::info, "No gossip backlog; proceeding");
    }
}
...
future<> gossiper::wait_for_range_setup() const {
    LOGMACRO(logger, log_level::info, "Waiting for pending range setup...");
...
bool gossiper::is_safe_for_bootstrap(inet_address endpoint) const {
    ...
    if (!host_id) {
        LOGMACRO(logger, log_level::debug, "is_safe_for_bootstrap: node={}, status=no state in gossip, allowed_to_bootstrap={}", endpoint, allowed);
        return allowed;
    }
    auto eps = get_endpoint_state_ptr(*host_id);
    if (!eps) {
    ...
    LOGMACRO(logger, log_level::debug, "is_safe_for_bootstrap: node={}, status={}, allowed_to_bootstrap={}", endpoint, status, allowed);
    return allowed;
}
...
bool gossiper::is_safe_for_restart(locator::host_id host_id) const {
    ...
    for (auto& x : _endpoint_state_map) {
        ...
        try {
            ...
            LOGMACRO(logger, log_level::debug, "is_safe_for_restart: node with host_id={}, status={}", node, status);
            ...
            LOGMACRO(logger, log_level::info, "is_safe_for_restart: node={} doesn't not have status or host_id yet in gossip", node);
        }
    }
    return allowed;
}
...
std::set<sstring> gossiper::get_supported_features(const std::unordered_map<locator::host_id, sstring>& loaded_peer_features, ignore_features_of_local_node ignore_local_node) const {
    ...
    for (auto& x : _endpoint_state_map) {
        ...
        if (ignore_local_node && host_id == my_host_id()) {
            LOGMACRO(logger, log_level::debug, "Ignore SUPPORTED_FEATURES of local node: features={}", features);
            continue;
        }
        if (features.empty()) {
            auto it = loaded_peer_features.find(host_id);
            if (it != loaded_peer_features.end()) {
                LOGMACRO(logger, log_level::info, "Node {} does not contain SUPPORTED_FEATURES in gossip, using features saved in system table, features={}", host_id, feature_service::to_feature_set(it->second));
            } else {
...
void gossiper::check_knows_remote_features(std::set<std::string_view>& local_features, const std::unordered_map<locator::host_id, sstring>& loaded_peer_features) const {
    ...
    if (boost::range::includes(local_features, common_features)) {
        LOGMACRO(logger, log_level::info, "Feature check passed. Local node {}/{} features = {}, Remote common_features = {}",
