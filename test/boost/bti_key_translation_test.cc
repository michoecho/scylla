...
static bool allows_empty_value(const abstract_type& type) {
    return type.bound_value_needs_to_be_reserialized() == false && type.is_value_compatible_with(type);
}
...
static managed_bytes make_random_value(std::mt19937& engine, tests::value_generator& value_gen, const abstract_type& type) {
    ...
    return managed_bytes(managed_bytes_view(bytes_view(value.serialize_nonnull())));
}
...
BOOST_AUTO_TEST_CASE(test_comparable_bytes_from_compound_random_valid_data) {
    ...
    for (int i = 0; i < n_schemas; ++i) {
        ...
        for (int j = 0; j < n_keys_per_schema; ++j) {
            // Clustering *prefixes* of any length are legal positions, so vary the length.
        ...
        for (size_t a = 0; a < entries.size(); ++a) {
            for (size_t b = 0; b < a; ++b) {
                const auto expected = pos_cmp(entries[a].pos(), entries[b].pos());
                const auto actual = entries[a].encoded <=> entries[b].encoded;
                if (expected != actual) {
