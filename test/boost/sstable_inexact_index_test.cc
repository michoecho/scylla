...
// in the Data file.
std::vector<sstables::sstable_datafile_position> get_ck_positions(shared_sstable sst, reader_permit permit) {
    std::vector<sstables::sstable_datafile_position> result;
...
std::vector<sstables::sstable_datafile_position> get_partition_positions(shared_sstable sst, reader_permit permit) {
    std::vector<sstables::sstable_datafile_position> result;
    {
        ...
        while (!ir->eof()) {
            result.push_back(ir->sstable_datafile_positions().start);
...
struct inexact_partition_index : abstract_index_reader {
    ...
    std::vector<sstables::sstable_datafile_position> _positions;
    std::span<const sstables::sstable_datafile_position> _pk_positions;
    std::span<const sstables::sstable_datafile_position> _ck_positions;
    ...
    inexact_partition_index(
        ...
        std::span<const sstables::sstable_datafile_position> pk_positions,
        std::span<const sstables::sstable_datafile_position> ck_positions,
        ...
        do {
            ...
            mutation_reader_assertions reader = assert_that(sstables::mx::make_reader(
                ...
                std::make_unique<inexact_partition_index>(
                    ncs,
                    partition_positions,
                    std::span<sstables::sstable_datafile_position>(),
                    present_dks,
