...
struct mock_consumer {
    ...
    mock_consumer(const schema& s, reader_permit permit, size_t depth) : _schema(s), _permit(std::move(permit)) {
        ...
        LOGMACRO(testlog, log_level::debug, "mock_consumer [{}] constructed: depth={}", fmt::ptr(this), _result._depth);
    }
    stop_iteration update_depth() {
        --_result._depth;
        auto stop = _result._depth < 1 ? stop_iteration::yes : stop_iteration::no;
        LOGMACRO(testlog, log_level::debug, "mock_consumer [{}]: update_depth: depth={} stop_iteration={}", fmt::ptr(this), _result._depth, stop);
        return stop;
    }
    void consume_new_partition(const dht::decorated_key& dk) {
        ++_result._consume_new_partition_call_count;
        LOGMACRO(testlog, log_level::debug, "mock_consumer [{}]: consume_new_partition: {}: {}", fmt::ptr(this), dk, _result._consume_new_partition_call_count);
    }
    stop_iteration consume(tombstone t) {
        ++_result._consume_tombstone_call_count;
        LOGMACRO(testlog, log_level::debug, "mock_consumer [{}]: consume tombstone: {}", fmt::ptr(this), _result._consume_tombstone_call_count);
        return stop_iteration::no;
    }
    stop_iteration consume(static_row&& sr) {
        LOGMACRO(testlog, log_level::debug, "mock_consumer [{}]: consume static_row", fmt::ptr(this));
        _result._fragments.emplace_back(_schema, _permit, std::move(sr));
        return update_depth();
    }
    stop_iteration consume(clustering_row&& cr) {
        LOGMACRO(testlog, log_level::debug, "mock_consumer [{}]: consume clustering_row: {}", fmt::ptr(this), cr.position());
        _result._fragments.emplace_back(_schema, _permit, std::move(cr));
        return update_depth();
    }
    stop_iteration consume(range_tombstone_change&& rtc) {
        LOGMACRO(testlog, log_level::debug, "mock_consumer [{}]: consume range_tombstone_change: {} {}", fmt::ptr(this), rtc.position(), rtc.tombstone());
        _result._fragments.emplace_back(_schema, _permit, std::move(rtc));
        return update_depth();
    }
    stop_iteration consume_end_of_partition() {
        ++_result._consume_end_of_partition_call_count;
        LOGMACRO(testlog, log_level::debug, "mock_consumer [{}]: consume_end_of_partition: {}", fmt::ptr(this), _result._consume_end_of_partition_call_count);
        return update_depth();
    }
    result consume_end_of_stream() {
        _result._consume_end_of_stream_called = true;
        LOGMACRO(testlog, log_level::debug, "mock_consumer [{}]: consume_end_of_stream", fmt::ptr(this));
