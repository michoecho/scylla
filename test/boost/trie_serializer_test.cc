...
size_t max_offset_from_child(const writer_node& x, size_t pos) {
    size_t result = 0;
...
node_type choose_node_type(const writer_node& x, size_t pos) {
...
public:
    ...
    virtual size_t write(const writer_node& x) override {
        auto before = pos();
    ...
    size_t write_impl(const writer_node& x) {
    ...
    virtual size_t serialized_size(const writer_node& x, size_t start_pos) const override {
