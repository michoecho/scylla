...
public:
    ...
    void set_values_for_leveled_strategy(uint64_t fake_data_size, uint32_t sstable_level, int64_t max_timestamp, sstring first_key, sstring last_key) {
        ...
        _sst->_components->summary.first_key.value = bytes(reinterpret_cast<const bytes::value_type*>(first_key.c_str()), first_key.size());
        _sst->_components->summary.last_key.value = bytes(reinterpret_cast<const bytes::value_type*>(last_key.c_str()), last_key.size());
